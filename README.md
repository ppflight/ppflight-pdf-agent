...
```bash
bash <(curl -fsSL --connect-timeout 20 --retry 2 https://raw.githubusercontent.com/ppflight/ppflight-pdf-agent/v1.0.8/bootstrap.sh)
```
...
- v1.0.6 及更早版本的 Tunnel 方案使用 Nginx；v1.0.8 新安装直接提供下载端口，无需 Nginx。`cloudflared` 请使用
...
sudo ./install.sh --version 1.0.8 --install-deps \
...
2. 安装不可变版本到 `/opt/ppflight-pdf-agent/releases/1.0.8`；
...
```bash
sudo ./install.sh --version 1.0.8 --install-deps --artifact-dir /srv/ppflight-pdf-artifacts
```
...
curl --fail --location --proto '=https' --tlsv1.2 \
  --output "$work_dir/ppflight-pdf-agent-1.0.8.tar.gz" \
  https://github.com/ppflight/ppflight-pdf-agent/releases/download/v1.0.8/ppflight-pdf-agent-1.0.8.tar.gz
curl --fail --location --proto '=https' --tlsv1.2 \
  --output "$work_dir/ppflight-pdf-agent-1.0.8.tar.gz.sha256" \
  https://github.com/ppflight/ppflight-pdf-agent/releases/download/v1.0.8/ppflight-pdf-agent-1.0.8.tar.gz.sha256
cd "$work_dir"
sha256sum -c ppflight-pdf-agent-1.0.8.tar.gz.sha256
tar -xzf ppflight-pdf-agent-1.0.8.tar.gz
cd ppflight-pdf-agent-1.0.8
sudo ./install.sh --version 1.0.8 --install-deps \
  --artifact-dir /srv/ppflight-pdf-artifacts
```
...
```bash
sudo ./update.sh --version 1.0.8 \
  --url https://github.com/ppflight/ppflight-pdf-agent/releases/download/v1.0.8/ppflight-pdf-agent-1.0.8.tar.gz \
...
./scripts/verify-release.sh --source . --version 1.0.8
```
