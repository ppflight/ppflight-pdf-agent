...
```bash
bash <(curl -fsSL --connect-timeout 20 --retry 2 https://raw.githubusercontent.com/ppflight/ppflight-pdf-agent/v1.0.13/bootstrap.sh)
```

脚本自动下载 v1.0.13、校验归档、安装缺失依赖、启动服务并引导绑定。
...
两者都只监听 `127.0.0.1`，不需要开放公网端口。v1.0.13 新安装由 Agent 原生提供 9761；
...
回到后台“账单文件设置”，确认 Agent 在线、版本为 `1.0.13`，再启用 PDF Agent 交付并保存。
...
可固定目标版本，例如 `sudo ag-pdf update 1.0.13`。该命令下载 Release 的 SHA-256、复用受控更新器并在健康检查失败时恢复上一版本，配置、绑定和既有 PDF 均保留。初次从 v1.0.11 升到 v1.0.13 时请按本 Release 页面提供的受控 `update.sh` 命令执行一次；该旧版的 `ag-pdf` 尚不含此子命令。

v1.0.11 首次升级可直接手动运行这一条 GitHub 命令，无需重新绑定：

```bash
bash <(curl -fsSL --connect-timeout 20 --retry 2 https://raw.githubusercontent.com/ppflight/ppflight-pdf-agent/v1.0.13/bootstrap.sh) --skip-bind
```
...
v1.0.13 在服务刚启动或重启时立即检查健康，尚未监听的早期尝试会显示该提示，然后自动重试。
...
- [v1.0.13 发行包](https://github.com/ppflight/ppflight-pdf-agent/releases/tag/v1.0.13)
