...
final class InvoiceRenderer
{
    ...
    private function html(array $snapshot): string
    {
        ...
        $summaryFields = static function (array $rows): string {
            $html = '<table class="summary-fields"><colgroup><col class="summary-key-col"><col></colgroup><tbody>';
            foreach ($rows as [$key, $fieldValue]) {
                $html .= '<tr><th class="summary-key" scope="row">' . $key . '</th><td class="summary-value">' . $fieldValue . '</td></tr>';
            }

            return $html . '</tbody></table>';
        };
        $leftSummary = $summaryFields([
            [$label('Reference', '账单编号'), $this->e($invoice['display_number'])],
            [$label('Issued', '开具日期'), $this->e($invoice['issued_at'])],
        ...
        return '<!doctype html><html lang="' . ($en ? 'en' : 'zh-CN') . '"><head><meta charset="utf-8"><style>' . $this->css() . '</style></head><body class="' . ($en ? 'locale-en' : 'locale-zh') . '">'
